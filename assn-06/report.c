...
{
  sem_wait(&(bank->report_transfer_locker));
  ...
  if (amountAbs < reportingAmount) {
    sem_post(&(bank->report_transfer_locker));
...
{
  sem_wait(&(bank->flag));
  bank->num_workers_left--;
  if(bank->num_workers_left != 0){
    sem_post(&(bank->flag));
    sem_wait(&(bank->stopped));
    return 0;
  }
  Report *rpt = bank->report;
