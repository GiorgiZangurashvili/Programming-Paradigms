          ...
          int numWorkers)
{
  ...
  bank-> num_workers_left = numWorkers; 
  sem_init(&(bank->flag), 0, 1); 
  sem_init(&(bank->stopped), 0, 0);
  sem_init(&(bank->report_transfer_locker), 0, 1); 
...
Bank_Balance(Bank *bank, AccountAmount *balance)
{
  ...
  for(unsigned int i = 0; i < bank->numberBranches; i++){
    sem_wait(&(bank->branches[i].branch_locker));
  }
  ...
  for (unsigned int branch = 0; branch < bank->numberBranches; branch++) {
    ...
    if (err < 0) {
      for(unsigned int i = 0; i < bank->numberBranches; i++){
        sem_post(&(bank->branches[i].branch_locker));
      }
      return err;
    }
