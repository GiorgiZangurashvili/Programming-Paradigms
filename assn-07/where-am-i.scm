...
;; of first circle and the rest of them
(define (helper_intersection circle circles_lst)
  (if (null? circles_lst) '()
    (append (intersect circle (car circles_lst))
      (helper_intersection circle (cdr circles_lst)))))

;; Finds all intersection points of circles 
;; in circles_lst list of circles
(define (intersection-points circles_lst)
  (if (null? circles_lst) '()
    (append (helper_intersection (car circles_lst) (cdr circles_lst))
      (intersection-points (cdr circles_lst)))))

;;Finds distances between a point and points 
;;in a list and then computes product of distances
(define (distance-product pt pt_lst)
  (apply * (map (lambda (elem) (dist pt elem))(remove pt pt_lst))))


;;Rates each point in list and annotates them
(define (rate-points pt_lst)
  (map (lambda (pt) (cons (distance-product pt pt_lst) (list pt))) pt_lst))

;;Compare function
(define (compare pt1 pt2)
  (cond ((< (car pt1) (car pt2)) -1)
        ((> (car pt1) (car pt2)) 1)
        (else 0)))

;;Sorts points with respect to their rates
(define (sort-points rated_pt_lst)
  (quicksort rated_pt_lst compare))

;;Rates points in a list, then sorts them and takes first half points of a list
(define (clumped-points pt_lst)
  (map (lambda (elem) (cadr elem)) (prefix-of-list (sort-points (rate-points pt_lst)) (quotient (length pt_lst) 2))))

;;Computes average of numbers in given list
(define (avg lst)
  (/ (apply + lst) (length lst)))

;;Computes coordinates of average point in a list 
;;and then annotates it with distance-product
(define (average-point pt_lst)
  (let ((avg-pt (make-pt (avg (map x pt_lst)) (avg (map y pt_lst)))))
    (cons (distance-product avg-pt pt_lst) (list avg-pt))))   

;;Computes intersection points in circles_lst list, then clumped-points 
;;and then average-point
(define (best-estimate circles_lst)
  (average-point (clumped-points (intersection-points circles_lst))))

;;Sorts points of a list
(define (where-am-i dist_lst loc_lst)
  (sort-points (map best-estimate (all-guesses dist_lst loc_lst))))
...
(define (partition pivot num-list comparefn)
  (if (null? num-list) '(() ())
      (let ((split-of-rest (partition pivot (cdr num-list) comparefn)))
	(if (< (comparefn (car num-list) pivot) 0)
...
(define (quicksort num-list comparefn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) comparefn)))
	(append (quicksort (car split) comparefn) 
		(list (car num-list)) 
		(quicksort (cadr split) comparefn)))))
